...
const valueItem = `{"type":"Values","id":"Values","children":[],"caption":"Values","originalItem":{"HIERARCHY_UNIQUE_NAME":"Values"},"filters":null}`;

export const use3dChartStore = defineStore("3dChart", () => {
  ...
  async function getMDX() {
    if (state.value.measures.length !== 3) return;
    ...
    const columns = queryDesignerStore.columns.filter(
      (e) => e.type !== "Values",
    );
    columns.push(JSON.parse(valueItem));
    const measures = state.value.measures;
  ...
  const setMeasures = (measures) => {
    const constructedMeasures = [] as any[];

    constructedMeasures.push({
      type: 5,
      caption: measures.xMeasure.MEASURE_NAME,
      id: null,
      originalItem: measures.xMeasure,
    });

    constructedMeasures.push({
      type: 5,
      caption: measures.yMeasure.MEASURE_NAME,
      id: null,
      originalItem: measures.yMeasure,
    });

    constructedMeasures.push({
      type: 5,
      caption: measures.zMeasure.MEASURE_NAME,
      id: null,
      originalItem: measures.zMeasure,
    });

    state.value.measures = constructedMeasures;
    fetchPivotTableData();
  };
