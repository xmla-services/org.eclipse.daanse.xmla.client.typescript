...
function getChildLevels(
  ...
  const levels = storage.levels
    .filter(
      (level) => level.HIERARCHY_UNIQUE_NAME === hierarchy.HIERARCHY_UNIQUE_NAME
    )
    .map((level) => {
...
function getLevelProperties(level: MDSchemaLevel, storage: MetadataStorage) {
  const levelProperties = storage.properties
    .filter(
      (property) => property.LEVEL_UNIQUE_NAME === level.LEVEL_UNIQUE_NAME
    )
    .map((property) => {
...
function getMembers(
  ...
  const extendedMembers = currentlyLoadedMembers.members.map((member) => {
    ...
    if (member.hasChildren) {
      if (childMembersLoaded && membersState.opened) {
        ...
        const loadingPlaceHolder = TreeViewItems.getLoadingItemDesc(member.id);
  ...
  if (parsedMembersDesc.hasMore) {
    const loadMoreButton = TreeViewItems.getLoadMoreItemDesc(levelTreeItemId);
...
function getChildMembers(memberTreeItemId: string) {
  ...
  if (parsedMembersDesc.hasMore) {
    const loadMoreButton = TreeViewItems.getLoadMoreItemDesc(memberTreeItemId);
    result.members.push(loadMoreButton);
  }
